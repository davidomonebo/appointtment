...
const MyAppointments = () => {
  const { doctors } = useContext(AppContext); // Make sure 'doctors' is provided in AppContext
        ...
        {doctors.slice(0, 3).map((item, index) => ( 
          <div key={index} className='py-4'>
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:gap-6 py-2 border-b'>
              <img className='w-32 h-32 object-cover bg-indigo-50' src={item.image} alt={item.name} />
              <div className='flex-1 text-sm text-zinc-600'>
                ...
                <p className='text-xs mt-1'>
                  ...
                  25, July, 2025 | 8:30 PM {/* Replace with dynamic date if needed */}
                </p>
              </div>
